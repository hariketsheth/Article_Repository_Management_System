...
function ChangeRole($con,$id, $role){
    ...
    $query="UPDATE users SET role_2='$role' WHERE username='$id'";
...
function ChangeStatus($con,$id, $role){
    $status = ($role == "block")?"inactive":"active";
    $query="UPDATE users SET role_1='$status' WHERE username='$id'";
    $run=mysqli_query($con,$query);
}
...
function getFinalBookmark($con, $user){

    $query="SELECT * FROM bookmarks WHERE username='$user'";
    $run=mysqli_query($con,$query);
    $data = array();
